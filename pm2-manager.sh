...
# ─── Resolve service name from user input ─────────────────────────────
...
ECOSYSTEMEOF
    echo -e "${GREEN}✅ Ecosystem config generated${NC}"
}
...
cmd_start() {
    ...
    if [ -z "$target" ]; then
        ...
        if is_running "$target"; then
            echo -e "${YELLOW}⚠️  ${target} is already running${NC}"
        ...
        echo -e "${BLUE}🚀 Starting ${target}...${NC}"
        setup_logrotate
        generate_ecosystem
        ...
        echo -e "${GREEN}✅ ${target} started${NC}"
        pm2 list | grep "$target"
    fi
}
...
cmd_stop() {
    ...
    for svc in "${services[@]}"; do
        if ! service_exists "$svc"; then
            echo -e "${YELLOW}⚠️  ${svc} is not registered${NC}"
            continue
        fi
        if ! is_running "$svc"; then
            echo -e "${YELLOW}⚠️  ${svc} is already stopped${NC}"
            continue
        fi
...
cmd_restart() {
    ...
    if [ -z "$target" ]; then
        ...
        pm2 save
        echo -e "${GREEN}✅ ${target} restarted${NC}"
        pm2 list | grep "$target"
    fi
}
    ...
    for svc in "${services[@]}"; do
        if service_exists "$svc"; then
            ...
            if is_running "$svc"; then
                ...
                echo -e "   ${YELLOW}○ ${svc} is stopped${NC}"
            fi
        else
            echo -e "   ${RED}✗ ${svc} is not registered${NC}"
        fi
    done
...
cmd_logs() {
    ...
    if [ -z "$target" ]; then
        ...
        if ! service_exists "$target"; then
            echo -e "${RED}❌ ${target} is not registered${NC}"
            return 1
        fi
        echo -e "${BLUE}📋 Showing ${target} logs (Ctrl+C to exit):${NC}"
        pm2 logs "$target"
    fi
}
