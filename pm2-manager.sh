...
ECOSYSTEMEOF
    echo -e "${GREEN}✅ Ecosystem config generated at ${ECOSYSTEM_FILE}${NC}"
}
...
start() {
    ...
    if service_exists; then
        echo -e "${YELLOW}⚠️  Found stopped ${SERVICE_NAME}. Removing it first...${NC}"
    ...
    echo -e "${GREEN}✅ ${SERVICE_NAME} started on port ${DEFAULT_PORT}${NC}"
    pm2 list | grep "$SERVICE_NAME"
}
...
stop() {
    ...
    if ! is_running; then
        echo -e "${YELLOW}⚠️  ${SERVICE_NAME} is already stopped${NC}"
        pm2 list | grep "$SERVICE_NAME"
        return 1
    fi
...
restart() {
    if ! service_exists; then
        echo -e "${YELLOW}⚠️  ${SERVICE_NAME} is not registered. Starting...${NC}"
        start
        return
    fi
    ...
    if ! is_running; then
        echo -e "${YELLOW}⚠️  Service is stopped. Removing and starting fresh...${NC}"
        pm2 delete "$SERVICE_NAME" 2>/dev/null || true
        cd "$SCRIPT_DIR"
        pm2 start ecosystem.config.js --update-env
    else
...
restart_all() {
    echo -e "${BLUE}🔄 Full restart: deleting logs and starting fresh...${NC}"
    ...
    pm2 save
    echo -e "${GREEN}✅ ${SERVICE_NAME} restarted fresh with clean logs${NC}"
    pm2 list | grep "$SERVICE_NAME"
}
...
status() {
    ...
    if service_exists; then
        ...
        if is_running; then
            ...
            echo -e "${YELLOW}⚠️  Service is registered but not running${NC}"
        fi
    else
...
logs() {
    ...
    if ! is_running; then
        echo -e "${YELLOW}⚠️  ${SERVICE_NAME} is not running. Showing last logs...${NC}"
    fi

    echo -e "${BLUE}📋 Showing ${SERVICE_NAME} logs (Ctrl+C to exit):${NC}"
