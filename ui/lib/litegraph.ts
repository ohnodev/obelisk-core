 ...
 */
export function serializeGraph(graph: LGraph): WorkflowGraph {
  const nodes: WorkflowNode[] = [];
  const connections: WorkflowConnection[] = [];

  // Serialize nodes - access nodes array directly
  const graphNodes: LGraphNode[] = (graph as any)._nodes || [];
 ...
 */
export function deserializeGraph(graph: LGraph, workflow: WorkflowGraph): void {
  graph.clear();
